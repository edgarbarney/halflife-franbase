...
ScorePanel::ScorePanel(int x, int y, int wide, int tall) : Panel(x, y, wide, tall)
{
	...
	{
		if ((g_ColumnInfo[i].m_pTitle != nullptr) && g_ColumnInfo[i].m_pTitle[0] == '#')
			m_HeaderLabels[i].setText(CHudTextMessage::BufferedLocaliseTextString(g_ColumnInfo[i].m_pTitle));
		else if (g_ColumnInfo[i].m_pTitle != nullptr)
...
void ScorePanel::Update()
{
	...
	// Set the title
	if (gViewPort->m_szServerName != nullptr)
	{
...
void ScorePanel::SortPlayers(int iTeam, char* team)
{
	...
	while (true)
	{
		...
		{
			if (m_bHasBeenSorted[i] == false && (g_PlayerInfoList[i].name != nullptr) && g_PlayerExtraInfo[i].frags >= highest_frags)
			{
				cl_entity_t* ent = gEngfuncs.GetEntityByIndex(i);

				if ((ent != nullptr) && !((team != nullptr) && stricmp(g_PlayerExtraInfo[i].teamname, team)))
				{
		...
		for (int col = 0; col < NUM_COLUMNS; col++)
		{
			...
			else
			{
				...
				case COLUMN_CLASS:
					...
					if ((gViewPort != nullptr) && EV_TFC_IsAllyTeam(g_iTeamNumber, g_PlayerExtraInfo[m_iSortedRows[row]].teamnumber))
						bShowClass = true;
...
void ScorePanel::mousePressed(MouseCode code, Panel* panel)
{
	...
	else if (m_iHighlightRow >= 0)
	{
		...
		if (iPlayer > 0)
		{
			...
			if ((pl_info != nullptr) && (pl_info->name != nullptr) && '\0' != pl_info->name[0])
			{
				char string[256];
...
void ScorePanel::MouseOverCell(int row, int col)
{
	...
	if ((pl_info->name == nullptr) || '\0' == pl_info->name[0])
		return;
