...
#if _DEBUG
	if (ScheduleFromName(pNewSchedule->pName) == nullptr)
	{
		...
		if (!FScheduleValid() || m_MonsterState != m_IdealMonsterState)
		{
				...
				(m_IdealMonsterState != MONSTERSTATE_SCRIPT || m_IdealMonsterState == m_MonsterState))
			{
				...
				if ((0 != m_afConditions && !HasConditions(bits_COND_SCHEDULE_DONE)) ||
					((m_pSchedule != nullptr) && (m_pSchedule->iInterruptMask & bits_COND_SCHEDULE_DONE) != 0) ||
...
void CBaseMonster::StartTask(Task_t* pTask)
{
	...
	case TASK_FIND_COVER_FROM_BEST_SOUND:
	{
		...
		if ((pBestSound != nullptr) && FindCover(pBestSound->m_vecOrigin, g_vecZero, pBestSound->m_iVolume, CoverRadius()))
		{
			// then try for plain ole cover
			m_flMoveWaitFinished = gpGlobals->time + pTask->flData;
			TaskComplete();
	...
	case TASK_WALK_TO_SCRIPT:
	{
		...
		if ((m_pGoalEnt == nullptr) || (m_pGoalEnt->pev->origin - pev->origin).Length() < 1)
			TaskComplete();
		else
		{
	...
	case TASK_GET_PATH_TO_BESTSOUND:
	{
		...
		if ((pSound != nullptr) && MoveToLocation(m_movementActivity, 2, pSound->m_vecOrigin))
		{
			TaskComplete();
		}
		else
	...
	case TASK_GET_PATH_TO_BESTSCENT:
	{
		...
		if ((pScent != nullptr) && MoveToLocation(m_movementActivity, 2, pScent->m_vecOrigin))
		{
			TaskComplete();
		}
		else
