...
void V_CamAnims(struct ref_params_s* pparams, cl_entity_s* view)
{
	...
	if (index == -1)
	{
		for (int i = 0; i < g_viewinfo.phdr->numbones; i++)
		{
			...
			else if (!stricmp(pbone[i].name, "gun") || !stricmp(pbone[i].name, "weapon") || !stricmp(pbone[i].name, "glock") || !stricmp(pbone[i].name, "Bip01 R Wrist") || !stricmp(pbone[i].name, "Bip01 R Hand"))
			{
				index = i;
				break;
			}
	...
	if (index != -1 && index < g_viewinfo.phdr->numbones)
	{
		...
		{
			result = g_viewinfo.boneangles[index] - g_viewinfo.prevboneangles[index];
		}
		result2 = g_viewinfo.bonepos[index] - g_viewinfo.prevbonepos[index];
